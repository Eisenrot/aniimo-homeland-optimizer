...
if(BASE_PALS.length!==98)throw new Error(`selectable AniIDEX basic roster drifted: ${BASE_PALS.length}`);
if(FORM_PALS.length!==123)throw new Error(`regional/Prismana form count drifted: ${FORM_PALS.length}`);
if(PALS.length!==221)throw new Error(`expanded roster should have 221 selectable entries, got ${PALS.length}`);
...
const unavailableSpecies=['Reefish','Coraliz','Cheekie','Wavwal','Bubbeep','Glameep','Popapus','Gachapus','Malangel','Malevsera'];
for(const name of unavailableSpecies){
  const p=base(name);
  if(!p||!p.unavailable)throw new Error(`${name} should be unavailable`);
  for(const form of FORM_PALS.filter(x=>x.speciesName===name))if(!form.unavailable)throw new Error(`${form.name} should inherit unavailable status`);
}
if(BASE_PALS.some(p=>p.name==='Fennelun'||p.name==='Soleon')||PALS.some(p=>p.name==='Fennelun'||p.name==='Soleon'))throw new Error('combat-only ult transformations must not appear in the selectable roster');
...
if(!/\.aniimo-hex-frame\{[\s\S]*?overflow:hidden;[\s\S]*?-webkit-mask:url\('https:\/\/aniidex\.com\/images\/aniimo\/ui\/hex-bg\.webp'\)[\s\S]*?mask:url\('https:\/\/aniidex\.com\/images\/aniimo\/ui\/hex-bg\.webp'\)/.test(portraitCss))throw new Error('Aniimo frame wrapper must use the AniIDEX hex background as its alpha mask');
if(/\.aniimo-hex-cut\{[\s\S]*?clip-path:polygon\(/.test(portraitCss))throw new Error('inner Aniimo cut should not use the old approximate polygon mask');
...
if(!app.includes("if(p.unavailable)x.owned[id].enabled=false"))throw new Error('unavailable Aniimo must be forced disabled during state normalization');
if(!app.includes("state.owned[String(p.id)].enabled=!p.unavailable"))throw new Error('Own All must not enable unavailable Aniimo');
if(!app.includes("p.unavailable?'disabled':''"))throw new Error('unavailable ownership controls should be disabled');
