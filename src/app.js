...
function fullLayoutSvg(layout,climate){
  ...
  const plotFills=allPlots.map(p=>{const locked=p.plot>level,off=!locked&&disabled.has(p.plot),color=plotPalette[p.plot-1],fillAlpha=locked?.055:off?.15:.27,textAlpha=locked?.22:off?.48:.72,cls=locked?'locked':off?'disabled':'enabled';return `<g class="layout-plot ${cls}" data-plot="${p.plot}" style="--plot-color:${color};--plot-fill-opacity:${fillAlpha};--plot-text-opacity:${textAlpha}"><rect class="layout-plot-fill" x="${p.x}" y="${p.y}" width="${p.w}" height="${p.h}"/><text x="${p.x+p.w/2}" y="${p.y+p.h/2+.65}" text-anchor="middle">${p.plot}</text></g>`;}).join('');
