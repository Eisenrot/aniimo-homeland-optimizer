...
import {
  ...
  diagnoseObjective,defaultRecipeEfficiencyPct,utilityTasksForScenario
} from './optimizer.js';
...
const LEGACY_FORM_IDS=new Map(DATA.pals.filter(p=>p.isForm&&p.legacyId!=null).map(p=>[String(p.legacyId),String(p.id)]));
const canonicalPalId=id=>LEGACY_FORM_IDS.get(String(id))||String(id);
...
function normalizeState(s){
  ...
  for(const p of DATA.pals){
    ...
    if(!x.owned[id])x.owned[id]={enabled:!p.isForm&&!p.unavailable,count:1};
    if(p.unavailable)x.owned[id].enabled=false;
  }
  return x;
}
...
function applyUrl(input){
  ...
  if(q.has('p')){const ids=new Set(q.get('p').split('.').map(canonicalPalId));for(const p of DATA.pals)state.owned[String(p.id)]={enabled:ids.has(String(p.id)),count:1};}
  const ownedCounts=q.get('oc')||q.get('c');if(ownedCounts)for(const token of ownedCounts.split('.')){const[rawId,n]=token.split('-'),id=canonicalPalId(rawId);if(state.owned[id])state.owned[id].count=Math.max(1,Number(n)||1);}
...
function renderLiving(){const groups=livingFacilityGroups(state,DATA),caught=groups.filter(g=>g.owned.length).length;$('#living-panel').innerHTML=title('Aniimo living in facilities',`${caught} of ${groups.length} caught`)+`<p class="micro" style="margin:0 0 8px">No second checklist. This is inferred from the Aniimo roster in the header. If a family is disabled, its resident-only recipes simply stop existing.</p><div class="living-grid">${groups.map(g=>{const fac=facilityMap.get(g.facility),outs=g.outputs.map(x=>x.name).join(' · '),who=g.owned.map(p=>p.name).join(','),pal=DATA.pals.find(p=>!p.isForm&&Math.floor(Number(p.id)/1000)===Math.floor(Number(g.pet)/1000))||{id:g.pet,name:g.petName,abilities:{}};return`<div class="living-card ${g.owned.length?'':'locked'}">${aniimoHex(pal,'living-aniimo',56,!!g.owned.length)}<div><b>${esc(g.petName)}</b><small>${esc(fac?.name||g.facility)} · makes ${esc(outs)}</small><div class="living-state">${g.owned.length?`available via ${esc(who)}`:'family not enabled'}</div></div></div>`;}).join('')}</div>`;}
...
function bindOwnership(){
  ...
  $('#own-all').onclick=()=>{for(const p of DATA.pals)state.owned[String(p.id)].enabled=!p.unavailable;saveState();renderOwnership();renderLiving();scheduleCompute();};
...
function aniimoHex(p,className='',size=27,active=true){const[a]=palFallbackColors(p),src=palHeadIcon(p),label=esc(p?.name||p?.speciesName||'Aniimo'),prismana=p?.isPrismana?'prismana':'';return `<span class="aniimo-hex ${prismana} ${active?'active':''} ${className}" style="--aniimo-size:${Number(size)||27}px;--aniimo-theme:${a}" title="${label}"><span class="aniimo-hex-ring" aria-hidden="true"></span><span class="aniimo-hex-frame"><span class="aniimo-hex-cut"><img src="${src}" alt="${label}" onerror="${palImageError(p)}"></span></span></span>`;}
function palAbilityChip(p,ability,size=19){const[a,b]=palFallbackColors(p),src=palHeadIcon(p);return `<span class="pal-ability-chip" data-palette-src="${src}" style="--pal-a:${a};--pal-b:${b}">${aniimoHex(p,'chip-aniimo',size,true)}<span><b>${esc(p.name)}</b> · Lv.${p.abilities[ability]}</span></span>`;}
function teamPalChip(member,inner='',size=22){const p=member.pal||member,[a,b]=palFallbackColors(p),src=palHeadIcon(p);return `<span class="pal-ability-chip team-pal-chip" data-palette-src="${src}" style="--pal-a:${a};--pal-b:${b}">${aniimoHex(p,'team-aniimo',size,true)}<span class="team-pal-copy"><b>${esc(p.name)}</b>${inner}</span></span>`;}
...
function utilityAssignmentDetails(team,ev){
  return (ev?.utilityAssignments||[]).map(a=>({...a,member:team?.[a.worker]||null}));
}
function utilityStaffingMarkup(plan,{team=null,ev=null,compact=false}={}){
  const tasks=utilityTasksForScenario(plan?.scenario||{});if(!tasks.length)return'';
  const exact=new Map(utilityAssignmentDetails(team,ev).map(a=>[a.task.key,a]));
  return `<div class="utility-staffing ${compact?'compact':''}"><div class="subhead">Dedicated utility staffing · ${tasks.length} Aniimo</div><div class="utility-staffing-grid">${tasks.map(task=>{
    const fac=facilityMap.get(task.facility),assigned=exact.get(task.key),candidates=assigned?[]:utilityCandidatePals(task,compact?2:3);
    return `<div class="utility-staff-card ${assigned?'assigned':''}">${fac?.icon?`<img class="utility-staff-facility" src="${asset(fac.icon)}" alt="">`:''}<div class="utility-staff-copy"><b>${esc(fac?.name||task.name)}</b><span>${abilityPill(task.ability,task.level,true)}</span></div><div class="utility-staff-worker">${assigned?.member?teamPalChip(assigned.member,`<small>${esc(task.ability)} utility</small>`,27):candidates.length?`<span class="utility-candidate-label">Candidates</span>${candidates.map(p=>palAbilityChip(p,task.ability,23)).join('')}`:`<span class="utility-missing">No enabled ${esc(task.ability)} Aniimo</span>`}</div></div>`;
  }).join('')}</div><p class="micro">Each active utility consumes one full station slot and one unique compatible Aniimo for the entire plan. These assignments cannot be shared with production, permanent jobs, or burst coverage.</p></div>`;
}
...
function staffingCoverageMarkup(summary){
  if(!summary)return'';
  const utility=[...(summary.utilityByFacility||new Map())],permanent=[...(summary.permanentByFacility||new Map())],burst=[...(summary.burstByFacility||new Map())],parts=[];
  if(utility.length)parts.push(`<div class="subhead">Utility 24/7 coverage</div><div class="coverage utility-coverage">${utility.map(([f,v])=>coverageChip(f,v)).join('')}</div>`);
  ...
  if(burst.length)parts.push(`<div class="subhead">Burst / restart coverage</div><div class="coverage burst-coverage">${burst.map(([f,v])=>burstCoverageChip(f,v)).join('')}</div><p class="micro burst-help">Ability coverage is reusable across sequential crop/tree jobs. Load is the current plan's real manual labour demand; capacity is each facility family's independent headroom after utility and permanent workers are reserved.</p>`);
  return parts.join('');
}
...
function renderPlan(){const rawRows=[...(currentPlan.rows||[])],rows=groupedPlanRows(rawRows),targetRate=state.target==='coin'?currentPlan.ratePerHour:currentPlan.targetRate;let labor=currentPlan.utilityWorkers||0;for(const r of rawRows)labor+=r.recipe.pet?r.units:(r.manualSeconds||0)*r.batchesPerHour/3600;const aside=currentPlan.realTeamApplied?`${rawRows.length} assignments · exact real-team + personalities`:`${rawRows.length} assignments · ${currentPlan.testedScenarios||1} utility scenarios tested`,progress=lastOptimizerStats?optimizerProgressMarkup({...lastOptimizerStats,scenarioIndex:lastOptimizerStats.scenarioTotal||currentPlan.testedScenarios||0,scenarioTotal:lastOptimizerStats.scenarioTotal||currentPlan.testedScenarios||0,progress:1,candidatesPerSecond:Number(lastOptimizerStats.candidatePlans||0)/(Math.max(1,Number(lastOptimizerStats.elapsedMs||0))/1000)},false):'';$('#plan-panel').innerHTML=title('Best plan',aside)+progress+`
  ...
  ${utilityStaffingMarkup(currentPlan,currentPlan.utilityStaffing?{team:currentPlan.utilityStaffing.team,ev:currentPlan.utilityStaffing.ev,compact:true}:{compact:true})}
...
function fullLayoutSvg(layout,climate){
  ...
  const plotFills=allPlots.map(p=>{const locked=p.plot>level,off=!locked&&disabled.has(p.plot),color=plotPalette[p.plot-1],fillAlpha=locked?.018:off?.03:.040,textAlpha=locked?.16:off?.26:.58,cls=locked?'locked':off?'disabled':'enabled',label=locked?`Plot ${p.plot} · locked until RV ${p.plot}`:off?`Plot ${p.plot} · disabled`:`Plot ${p.plot} · enabled`;return `<g class="layout-plot ${cls}" data-plot="${p.plot}" style="--plot-color:${color};--plot-fill-opacity:${fillAlpha};--plot-text-opacity:${textAlpha}"><title>${label}</title><rect class="layout-plot-fill" x="${p.x}" y="${p.y}" width="${p.w}" height="${p.h}"/><text x="${p.x+p.w/2}" y="${p.y+p.h/2+.65}" text-anchor="middle">${p.plot}</text></g>`;}).join('');
  ...
  const plotLines=allPlots.map(p=>{const color=plotPalette[p.plot-1],locked=p.plot>level,off=!locked&&disabled.has(p.plot),strokeAlpha=locked?.16:off?.22:.82;return `<rect class="layout-plot-outline ${locked?'locked':off?'disabled':'enabled'}" style="--plot-color:${color};--plot-outline-opacity:${strokeAlpha}" x="${p.x}" y="${p.y}" width="${p.w}" height="${p.h}"/>`;}).join('');
