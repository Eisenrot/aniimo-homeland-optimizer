...
function renderLiving(){const groups=livingFacilityGroups(state,DATA),caught=groups.filter(g=>g.owned.length).length;$('#living-panel').innerHTML=title('Aniimo living in facilities',`${caught} of ${groups.length} caught`)+`<p class="micro" style="margin:0 0 8px">No second checklist. This is inferred from the Aniimo roster in the header. If a family is disabled, its resident-only recipes simply stop existing.</p><div class="living-grid">${groups.map(g=>{const fac=facilityMap.get(g.facility),outs=g.outputs.map(x=>x.name).join(' · '),who=g.owned.map(p=>p.name).join(', ');return`<div class="living-card ${g.owned.length?'':'locked'}"><img src="${palHeadIcon(DATA.pals.find(p=>!p.isForm&&Math.floor(Number(p.id)/1000)===Math.floor(Number(g.pet)/1000))||{id:g.pet})}" alt=""><div><b>${esc(g.petName)}</b><small>${esc(fac?.name||g.facility)} · makes ${esc(outs)}</small><div class="living-state">${g.owned.length?`available via ${esc(who)}`:'family not enabled'}</div></div></div>`;}).join('')}</div>`;}
...
const ANIIDEX_HEADS='https://aniidex.com/_ipx/q_95%26fit_inside%26s_60x60/images/aniimo';
