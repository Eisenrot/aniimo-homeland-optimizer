...
let state=loadState(),currentPlan=null,currentModel=null,recomputeTimer=null,undoSnapshot=null,undoTimer=null,teamSpeedOverride=null,teamAnalysisBaseline=null;
...
function clearTeamSpeedOverride(refresh=true){const had=!!teamSpeedOverride;teamSpeedOverride=null;teamAnalysisBaseline=null;if(had&&refresh)renderFacilities();return had;}
...
async function analyzeTeam(){const btn=$('#analyze-team'),status=$('#team-status'),box=$('#team-results');btn.disabled=true;box.innerHTML='';try{if(teamSpeedOverride){teamSpeedOverride=null;teamAnalysisBaseline=null;computePlan({keepTeam:true});renderFacilities();}teamAnalysisBaseline=captureTeamBaseline();const baseline=clone(teamAnalysisBaseline);const teams=await findBestTeams(currentModel,currentModel.state||state,DATA,{limit:4,onProgress:t=>status.innerHTML=`<span class="loader"></span> ${esc(t)}`});if(!teams.length)throw new Error('No valid team found.');const best=teams[0],core=findEssentialCore(currentModel,best.team,best.eval.objectiveRate),anti=antiStallSummary(currentModel,best.team,core,best.eval.rows);let special=null;for(let i=0;i<Math.min(3,teams.length);i++){status.innerHTML=`<span class="loader"></span> Measured-speed personality pass ${i+1}/${Math.min(3,teams.length)}`;const ev=await optimizePersonalities(currentModel,teams[i].team,t=>status.innerHTML=`<span class="loader"></span> ${esc(t)}`,teams[i].eval,teams[i].burst);if(!special||ev.objectiveRate>special.ev.objectiveRate)special={team:teams[i].team,ev};}teamSpeedOverride=state.manualSpeeds?null:speedOverrideFromSpecial(special);if(teamSpeedOverride){computePlan({keepTeam:true});renderFacilities();}status.textContent=state.manualSpeeds?`Done · manual Speed % stayed locked; comparison uses the pre-analysis theoretical baseline`:teamSpeedOverride?`Done · measured Efficiency applied; comparison stays frozen to the pre-analysis theoretical baseline`:`Done · actual roster optimum calculated against the frozen theoretical baseline`;box.innerHTML=(special?renderSpecial(special,baseline):'')+teams.map((x,i)=>renderTeamCard(x,i,i===0?{core,anti}:null,baseline)).join('');applyPalGradients();}catch(e){teamAnalysisBaseline=null;status.textContent=e.message||String(e);box.innerHTML=`<div class="empty warning">${esc(e.message||e)}</div>`;}finally{btn.disabled=false;}}
...
function renderAll(){teamSpeedOverride=null;teamAnalysisBaseline=null;renderObjectives();renderGeneral();renderFacilities();renderModules();renderNotes();renderLiving();renderOwnership();computePlan();}
...
if([...new URL(location.href).searchParams.keys()].some(k=>['f','m','y','a','h','w','tw','p','c','collect','one','cl','gen','hungry','sm','target','g','mx','gd','rn'].includes(k))){try{applyUrl(location.href);}catch{renderAll();}}else renderAll();
