...
function palAbilityChip(p,ability){const[a,b]=palFallbackColors(p),src=palHeadIcon(p);return `<span class="pal-ability-chip" data-palette-src="${src}" style="--pal-a:${a};--pal-b:${b}">${aniimoHex(p,'chip-aniimo',19,true)}<span><b>${esc(p.name)}</b> · Lv.${p.abilities[ability]}</span></span>`;}
function teamPalChip(member,inner=''){const p=member.pal||member,[a,b]=palFallbackColors(p),src=palHeadIcon(p);return `<span class="pal-ability-chip team-pal-chip" data-palette-src="${src}" style="--pal-a:${a};--pal-b:${b}">${aniimoHex(p,'team-aniimo',22,true)}<span class="team-pal-copy"><b>${esc(p.name)}</b>${inner}</span></span>`;}
