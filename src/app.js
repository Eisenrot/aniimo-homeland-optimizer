...
function groupedPlanRows(rows){
  ...
  return order.flatMap(facility=>groups.get(facility).sort((a,b)=>Number(b.perHour||0)-Number(a.perHour||0)).map((row,i)=>({row,continued:i>0})));
}
function renderPlan(){const rawRows=[...(currentPlan.rows||[])],rows=groupedPlanRows(rawRows),targetRate=state.target==='coin'?currentPlan.ratePerHour:currentPlan.targetRate;let labor=currentPlan.utilityWorkers||0;for(const r of rawRows)labor+=r.recipe.pet?r.units:(r.manualSeconds||0)*r.batchesPerHour/3600;$('#plan-panel').innerHTML=title('Best plan',`${rawRows.length} assignments · ${currentPlan.testedScenarios||1} utility scenarios tested`)+`
  ...
  ${currentPlan.infeasible?`<div class="empty warning">No feasible plan satisfies the current requirements.</div>`:rows.length?`<table class="plan-table"><thead><tr><th>Facility</th><th>Produce</th><th>Needs</th><th class="num">Cycle</th><th class="num coin-head"><img src="${HOME_COIN_ICON}" alt="">Coin/h</th></tr></thead><tbody>${rows.map(x=>planRow(x.row,x.continued)).join('')}</tbody></table><p class="micro">Estimated active labour: ${labor.toFixed(2)} Aniimo-hours per hour, including resident and utility assignments.</p>`:`<div class="empty">No runnable production chain with the current settings.</div>`}`);}
...
const SHARE_STATE_KEYS=['f','m','y','a','h','w','tw','p','c','collect','one','cl','gen','hungry','sm','target','g','mx','gd','rn'];
