import { DATA, maxAniimoForLevel, producedTargets } from '../state'
  ...
  return (
    <>
      <section className="panel">
        <div className="section-title"><span /><h3>Objective</h3><i /></div>
        <div className="field">
          <label>Primary target</label>
          <select
            value={state.target}
            onChange={(event) => patch((draft) => { draft.target = event.target.value })}
          >
        ...
        <p className="micro">MAX co-objectives and hard guarantees remain in the legacy reference for the next migration slice.</p>
      </section>
      ...
      <section className="panel">
        ...
        <div className="modern-checks">
          <label className="check-row compact-rule">
            ...
            />
            <span><b>One recipe per facility</b><small>Preserves the current solver rule.</small></span>
          </label>
          <label className="check-row compact-rule">
            <input
              type="checkbox"
        ...
        <div className="modern-utility-grid">
          {([
            ...
            <label className="utility-card" key={key}>
              <input
                type="checkbox"
